...
if __name__ == '__main__':
    ...
    result = Image.fromarray((predcolor * 255).astype(np.uint8))
    result.save(os.path.join(outdir, "filter_output_%d.png" % id))

    result = Image.fromarray((np.transpose(test_rgb[id], [1, 2, 0]) * 255).astype(np.uint8))
    result.save(os.path.join(outdir, "filter_input_rgb_%d.png" % id))
    
    result = Image.fromarray((test_grey[id, 0, :, :] * 255).astype(np.uint8))
    result.save(os.path.join(outdir, "filter_input_%d.png" % id))
