...
class MyDilatedConv2d(MyConv2d):
    ...
    def forward(self, input):
        ############### YOUR CODE GOES HERE ###############
        C_in, C_out, W, H = self.weight.shape

        W_new = 2 * self.dilation + W
        H_new = 2 * self.dilation + H
        weight = F.upsample(self.weight, size = (W_new, H_new))

        mask = torch.zeros((C_in, C_out, W_new, H_new))

        W_idx = torch.range(0, W_new - 1, step = self.dilation, dtype = torch.long)
        H_idx = torch.range(0, W_new - 1, step = self.dilation, dtype = torch.long)
        mask[:, :, W_idx, H_idx] = 1

        weight = torch.mul(weight, mask)

        self.padding = (W_new//2, H_new//2)

        return F.conv2d(input, weight, self.bias, padding = self.padding)
        ###################################################
...
class CNN(nn.Module):
    def __init__(self, kernel, num_filters, num_colors):
        ...
        self.upconv2 = nn.Sequential(
            MyConv2d(num_filters, num_colors, kernel_size=kernel, padding=padding),
            nn.Upsample(scale_factor=2),
            nn.BatchNorm2d(num_colors),
            nn.ReLU())

        self.finalconv = MyConv2d(num_colors, num_colors, kernel_size=kernel)
        ###################################################
...
class UNet(nn.Module):
    def __init__(self, kernel, num_filters, num_colors):
        ...
        self.finalconv = MyConv2d(num_colors + 1, num_colors, kernel_size=kernel)
        ###################################################
...
if __name__ == '__main__':
    ...
    print(args.kernel, args.num_filters, num_colors)    
    print(cnn)
    # 1/0
